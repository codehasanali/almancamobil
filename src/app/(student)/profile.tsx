  ...
  return (
    <ScrollView style={dashboardStyles.screen} contentContainerStyle={dashboardStyles.content}>
      ...
      <View style={dashboardStyles.card}>
        <Text style={dashboardStyles.title}>{user?.name ?? 'Mehmet Öğrenci'}</Text>
        <Text style={dashboardStyles.detail}>{user?.email ?? 'ogrenci@okul.com'}</Text>
        {user?.username ? <Text style={dashboardStyles.detail}>@{user.username}</Text> : null}
        <Text style={[dashboardStyles.detail, { marginTop: 10 }]}>Rol: Öğrenci</Text>
      </View>
